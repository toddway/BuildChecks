...
class Buildchecks < Formula
  ...
  url "https://github.com/toddway/BuildChecks/releases/download/v4.0.0/buildchecks-4.0.0-all.jar", using: :nounzip
  ...
  def install
    libexec.install "buildchecks-4.0.0-all.jar"
    # write_jar_script writes a bin/ wrapper that runs `java -jar <jar>` with the right JDK.
    bin.write_jar_script libexec/"buildchecks-4.0.0-all.jar", "buildchecks"
  end
